        ...
        if(excelRead.masterKeyMap.containsKey(queryKey)){
            for(String eachSubKey : excelRead.masterKeyMap.get(queryKey)){
                ...
                if( subKey.contains("DCALLOCATION") )
                {
                    ...
                    dbName.put(excelRead.queryKeyMap.get(eachSubKey.substring(0,eachSubKey.indexOf(":"))), "DCAllocation")

                }
                else if( subKey.contains("DCORDER") )
                {

                    dbName.put(excelRead.queryKeyMap.get(eachSubKey.substring(0,eachSubKey.indexOf(":"))), "DCOrder")

                }
                else if( subKey.contains("LMCORE") )
                {

                    dbName.put(excelRead.queryKeyMap.get(eachSubKey.substring(0,eachSubKey.indexOf(":"))), "lmCore")

                }
                else if( subKey.contains("WORKRELEASE") )
                {

                    dbName.put(excelRead.queryKeyMap.get(eachSubKey.substring(0,eachSubKey.indexOf(":"))), "WorkRelease")

                }
                // DI
                else if( subKey.contains("DEVICEINTEGRATION") )
                {

                    dbName.put(excelRead.queryKeyMap.get(eachSubKey.substring(0,eachSubKey.indexOf(":"))), "DIDatabase")
